...
def get_model(model):
    ...
    if model == 'hz_direction':
        ...
        tan = sympy.tan
        atan = sympy.atan
        wanted_model = atan((to_y - from_y)/(to_x - from_x)) - orientation_correction - true
    return wanted_model
...
get_A_row(get_model('hz_direction'), observations[0], points)
get_A_row(get_model('hz_direction'), observations[4], points)
